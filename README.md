...
| 26 | 26th Knesset (simulated scenario) | 27.10.2026 (estimated) | ~66.6% |

### Election 26 — Simulated Scenario

K26 is a feature-flagged simulated scenario behind `?e26=1`. It's not real data — the K26 ballot file is generated by `simulate_election_26.py`, which redistributes K25 ballots through a hand-crafted transfer matrix with per-source turnout factors, a global ~7.5% population-growth multiplier (Nov 2022 → Oct 2026), and Dirichlet noise per ballot.

The list map follows [Madad 120](https://madad120.geva.workers.dev/) as of 7 Sep 2026 (CEC letter-symbols still provisional until the official harvest):

- Passing: ישר 22, ליכוד 20, ביחד 13, הדמוקרטים 9, ישראל ביתנו 8, הרשימה המשותפת 8, יהדות התורה 8, ש״ס 7, עוצמה יהודית 7, הציונות הדתית 5, רע״ם 5, עמך ישראל 4, הנדל–זליכה 4
- Below threshold: כחול לבן (Gantz)

Edit `simulate_election_26.py`'s `TRANSFER_MATRIX`, `ROW_TURNOUT`, and `POP_GROWTH` to re-tune, then sync dhondt / wiki_official_results / party_config seats.
...
# Simulated Election 26 scenario (writes ballot26.csv + all downstream)
python simulate_election_26.py                          # alpha=55 (noise), seed=42
python prepare_election_26.py --real-csv ballot26.csv   # runs tsne/transfer/map for K26
```
