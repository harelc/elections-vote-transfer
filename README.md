...
| 26 | 26th Knesset (simulated scenario) | 27.10.2026 (estimated) | ~68.5% |

### Election 26 — Simulated Scenario

K26 is a feature-flagged simulated scenario behind `?e26=1`. It's not real data — the K26 ballot file is generated by `simulate_election_26.py`, which redistributes K25 ballots through a hand-crafted transfer matrix with per-source turnout factors, a global ~7.5% population-growth multiplier (Nov 2022 → Oct 2026), and Dirichlet noise per ballot. The scenario currently models:

- 14 parties total: Likud, ביחד (Beyachad — Bennett-Lapid joint list), ישר (Yashar — Eisenkot-led centrist), The Democrats (Labor+Meretz merge), Shas, Otzma Yehudit, UTJ, Yisrael Beitenu, Hadash-Ta'al, Ra'am
- Sub-threshold: Religious Zionism, המילואימניקים (Reservists — Hendel), Balad, Blue and White

Approximate seats: **25 / 22 / 16 / 11 / 10 / 9 / 8 / 8 / 6 / 5** (= 120). Edit `simulate_election_26.py`'s `TRANSFER_MATRIX`, `ROW_TURNOUT`, and `POP_GROWTH` to re-tune.
